...
# Same fix as app/main.py: Windows cp1252 console crashes logfire's exporter on
# emoji/unicode spans — force UTF-8 output.
    ...
    with logfire.span("Processing File", file=filename, source=source_type):
        try:
            ...
            with logfire.span("Vectorizing & Indexing"):
                ...
                # Upsert in small batches with retry — the cloud cluster times out
                # on large single writes (many 3072-dim vectors per request).
                UPSERT_BATCH = 20
                upsert_waits = [5, 15, 30]
                for i in range(0, len(points), UPSERT_BATCH):
                    batch_points = points[i : i + UPSERT_BATCH]
                    for attempt in range(len(upsert_waits) + 1):
                        try:
                            ...
                            if attempt < len(upsert_waits):
                                wait = upsert_waits[attempt]
                                logfire.warning(
                                    f"Qdrant upsert failed ({e}) — retrying in {wait}s "
                                    f"(attempt {attempt + 1}/{len(upsert_waits) + 1})."
                                )
                                time.sleep(wait)
                            else:
                                raise
